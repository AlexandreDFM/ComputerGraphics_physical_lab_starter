	...
	Fl_Gl_Window(x, y, w, h)
{
	...
	Mover *first_object = new Mover(cyclone::Vector3(-5, 3, 0), cyclone::Vector3(0, 0, 0), cyclone::Vector3(0, 0, 0), 1.0f, 1.0f);
	m_movers[first_object->getId()] = first_object;
	Mover *second_object = new Mover(cyclone::Vector3(0, 3, 0), cyclone::Vector3(0, 0, 0), cyclone::Vector3(0, 0, 0), 1.0f, 1.0f);
	m_movers[second_object->getId()] = second_object;
	Mover *third_object = new Mover(cyclone::Vector3(5, 3, 0), cyclone::Vector3(0, 0, 0), cyclone::Vector3(0, 0, 0), 1.0f, 1.0f);
...
void MyGlWindow::update()
{
	...
	if (m_connection) m_connection->updateColor(static_cast<float>(TimingData::get().lastFrameTimestamp));

	if (!m_movers.empty()) for (const auto mover : m_movers) mover.second->updateColor(static_cast<float>(TimingData::get().lastFrameTimestamp));

	if (duration <= 0.0f) return;

	if (!m_movers.empty()) for (const auto mover : m_movers) mover.second->update(duration);

	if (m_connection) m_connection->update(duration);
}
...
void MyGlWindow::doPick()
{
	...
	if (m_connection) m_connection->draw(0);
	...
	switch (e) {
		...
		case FL_DRAG: // if the user drags the mouse
			if (selected >= 0 && m_pressedMouseButton == 1) {
				...
				static_cast<double>(m_movers[selected]->m_particle->getPosition().z),
				rx, ry, rz,
				...
				const double fractionChangeX = static_cast<double>(Fl::event_x() - m_lastMouseX) / static_cast<double>(this->w());
				const double fractionChangeY = static_cast<double>(m_lastMouseY - Fl::event_y()) / static_cast<double>(this->h());
				...
				if (m_pressedMouseButton == 1) {
					...
					m_viewer->translate(-static_cast<float>(fractionChangeX), -static_cast<float>(fractionChangeY), (Fl::event_key(FL_Shift_L) == 0) || (Fl::event_key(FL_Shift_R) == 0));
				} else {
