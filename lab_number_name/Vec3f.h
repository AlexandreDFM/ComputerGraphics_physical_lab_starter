...
public:
    Vec3f(const float x = 0.f, const float y = 0.f, const float z = 0.f) : _x(x), _y(y), _z(z) {}
    Vec3f(const Vec3f &other) : _x(other._x), _y(other._y), _z(other._z) {}

    // Component setters (can be chained)
    inline Vec3f &x(const float x) {
        _x = x;
        return *this;
    }
    inline Vec3f &y(const float y) {
        _y = y;
        return *this;
    }
    inline Vec3f &z(const float z) {
        _z = z;
        return *this;
    }
    ...
    // Dot product
    inline float dot(const Vec3f &rhs) const { return _x * rhs._x + _y * rhs._y + _z * rhs._z; }

    // Cross product
    inline Vec3f cross(const Vec3f &rhs) const {
        return Vec3f(_y * rhs._z - _z * rhs._y, _z * rhs._x - _x * rhs._z, _x * rhs._y - _y * rhs._x);
    }
    ...
    inline void print(std::ostream &out = std::cout) const { out << "(" << _x << ", " << _y << ", " << _z << ") "; }

    // Returns the magnitude of the vector's components
    inline float magnitude() const { return sqrt(_x * _x + _y * _y + _z * _z); }
};

inline float dot(const Vec3f &lhs, const Vec3f &rhs) { return lhs.dot(rhs); }

inline Vec3f cross(const Vec3f &lhs, const Vec3f &rhs) { return lhs.cross(rhs); }
...
inline float magnitude(const Vec3f &v) { return v.magnitude(); }
...
inline Vec3f lerp(const float t, const Vec3f &p0, const Vec3f &p1) { return (p0 + t * (p1 - p0)); }
